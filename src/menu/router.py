...
async def get_submenus(menu_id: UUID, submenu: SubmenuService = Depends()) -> list[GetSubmenu]:
    return await submenu.all()


@router.get('/{menu_id}/submenus/{submenu_id}', response_model=GetSubmenu)
async def get_submenu(menu_id: UUID, submenu_id: UUID, submenu: SubmenuService = Depends()) -> GetSubmenu:
...
async def add_submenu(menu_id: str, new_menu: CreateMenu, submenu: SubmenuService = Depends()) -> GetSubmenu:
...
async def update_submenu(menu_id: UUID, submenu_id: UUID, updated_menu: CreateMenu, submenu: SubmenuService = Depends()) -> GetSubmenu:
...
async def get_dishes(menu_id: UUID, submenu_id: UUID, dish: DishService = Depends()) -> list[GetDish]:
    return await dish.all()


@router.get('/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}', response_model=GetDish)
async def get_dish(menu_id: UUID, submenu_id: UUID, dish_id: UUID, dish: DishService = Depends()) -> GetDish:
...
async def add_dish(menu_id: UUID, submenu_id: UUID, new_dish: CreateDish, dish: DishService = Depends()) -> GetDish:
...
async def update_dish(menu_id: UUID, submenu_id: UUID, dish_id: UUID, updated_dish: CreateDish, dish: DishService = Depends()) -> GetDish:
